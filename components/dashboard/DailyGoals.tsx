...
import {
  getTodayGoals, addGoals, toggleGoal, deleteGoal, parseXodusInput,
...
export default function DailyGoals() {
  ...
  function handleSubmit() {
    ...
    const result = parseXodusInput(trimmed)

    if (result.goals.length > 0) {
      addGoals(result.targetDate, result.goals)
    }

    if (result.nutritionUpdate) {
      saveNutritionProfile(result.nutritionUpdate)
    }
    ...
    if (result.nutritionUpdate) {
      parts.push('nutrition profile updated')
    }
    if (parts.length > 0) {
    ...
    <div className="rounded-2xl bg-[#111] border border-white/[0.07] p-4 space-y-3">
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder="train chest, finish parser, buy groceries..."
          className="flex-1 bg-[#0f0f0f] border border-white/[0.08] rounded-xl px-3 py-2 text-[11px] font-mono text-white placeholder-zinc-700 focus:outline-none focus:border-blue-500/40 min-w-0"
        />
        <button
          onClick={handleSubmit}
          className="flex-shrink-0 w-8 h-8 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-400 hover:bg-blue-500/20 transition-colors"
        >
          <Plus size={14} />
        </button>
      </div>
